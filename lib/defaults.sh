...
readonly DEFAULT_NAS_HOST="${NAS_MOUNT_DEFAULT_HOST:-192.168.54.249}"

# Default SMB port
readonly DEFAULT_SMB_PORT="${NAS_MOUNT_SMB_PORT:-445}"

# === Directory Structure Defaults ===
# Mount directory prefix (prepended to share names)
readonly DEFAULT_MOUNT_DIR_PREFIX="${NAS_MOUNT_DIR_PREFIX:-nas_}"

# Config directory name (relative to script directory)
readonly DEFAULT_CONFIG_DIR_NAME="${NAS_MOUNT_CONFIG_DIR:-config}"

# Log directory name (relative to script directory)
readonly DEFAULT_LOG_DIR_NAME="${NAS_MOUNT_LOG_DIR:-logs}"

# Default mount root location
readonly DEFAULT_MOUNT_ROOT="${NAS_MOUNT_ROOT:-$HOME/nas_mounts}"

# === File Name Defaults ===
# Credentials file name (stored in home directory)
readonly DEFAULT_CREDENTIALS_FILENAME="${NAS_MOUNT_CREDENTIALS_FILE:-.nas_credentials}"

# Config file name (stored in config directory)
readonly DEFAULT_CONFIG_FILENAME="${NAS_MOUNT_CONFIG_FILE:-config.sh}"

# Log file name (stored in log directory)
readonly DEFAULT_LOG_FILENAME="${NAS_MOUNT_LOG_FILE:-nas_mount.log}"

# === Service Name Defaults ===
# macOS LaunchAgent name
readonly DEFAULT_LAUNCHAGENT_NAME="${NAS_MOUNT_LAUNCHAGENT:-com.jpierce.nas-mounts}"

# Linux systemd service name
readonly DEFAULT_SYSTEMD_SERVICE_NAME="${NAS_MOUNT_SYSTEMD_SERVICE:-nas-mounts}"

# === Default Share List ===
# Space-separated list of default shares
readonly DEFAULT_SHARES="${NAS_MOUNT_DEFAULT_SHARES:-backups documents media notes PacificRim photos}"

# === GitHub Repository Defaults ===
# GitHub username
readonly DEFAULT_GITHUB_USER="${NAS_MOUNT_GITHUB_USER:-jdpierce21}"

# GitHub repository name
readonly DEFAULT_GITHUB_REPO="${NAS_MOUNT_GITHUB_REPO:-nas_mount}"

# GitHub branch
readonly DEFAULT_GITHUB_BRANCH="${NAS_MOUNT_GITHUB_BRANCH:-master}"

# === Mount Options Defaults ===
# macOS mount options
readonly DEFAULT_MACOS_MOUNT_OPTIONS="${NAS_MOUNT_MACOS_OPTIONS:--N -o nobrowse}"

# Linux mount options (template - uid/gid will be added dynamically)
readonly DEFAULT_LINUX_MOUNT_OPTIONS="${NAS_MOUNT_LINUX_OPTIONS:-iocharset=utf8,file_mode=0777,dir_mode=0777}"

# === Timeout Defaults ===
# Normal unmount timeout (in deciseconds - 30 = 3 seconds)
readonly DEFAULT_UNMOUNT_TIMEOUT="${NAS_MOUNT_UNMOUNT_TIMEOUT:-30}"

# Force unmount timeout (in deciseconds - 20 = 2 seconds)
readonly DEFAULT_FORCE_UNMOUNT_TIMEOUT="${NAS_MOUNT_FORCE_UNMOUNT_TIMEOUT:-20}"

# Mount verification wait time (seconds)
readonly DEFAULT_MOUNT_WAIT="${NAS_MOUNT_WAIT:-1}"

# Mount retry wait time (seconds)
readonly DEFAULT_MOUNT_RETRY_WAIT="${NAS_MOUNT_RETRY_WAIT:-2}"
...
readonly DEFAULT_DEBUG_LOG_RETENTION="${NAS_MOUNT_DEBUG_LOG_RETENTION:-10}"

# Test file prefix
readonly DEFAULT_TEST_FILE_PREFIX="${NAS_MOUNT_TEST_FILE_PREFIX:-.nas_mount_test_}"
